      ...
      <form @submit.prevent="handleSubmit" class="lg:col-span-7 card-premium p-6 flex flex-col gap-4 bg-white">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div class="flex flex-col gap-1">
            ...
            <div v-if="!previewUrl" class="flex flex-col gap-2 mt-1">
              <input 
                type="file" 
                ref="fileInput" 
                accept="image/*" 
              ...
              <button 
                ...
                class="flex items-center justify-center gap-2 border-2 border-dashed border-slate-200 hover:border-brand-400 rounded-2xl h-32 w-full text-slate-500 hover:text-brand-500 hover:bg-brand-50 transition-all group"
              >
                <div class="flex flex-col items-center gap-1">
                  <div class="w-10 h-10 rounded-full bg-slate-50 group-hover:bg-white flex items-center justify-center transition-colors">
            ...
            <!-- Creative Image Preview with Undo 'X' Button -->
            <div v-else class="relative w-full h-32 mt-1 rounded-2xl overflow-hidden group border-2 border-slate-100 shadow-sm">
              ...
              <div class="absolute inset-0 bg-slate-900/40 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center backdrop-blur-[2px]">
                <button 
                  type="button" 
                  @click="removeFile"
                  class="flex items-center gap-2 bg-rose-500 hover:bg-rose-600 text-white px-4 py-2 rounded-xl text-xs font-black shadow-lg transform translate-y-4 group-hover:translate-y-0 transition-all duration-300"
                >
            ...
            <span v-if="!previewUrl" class="text-[9px] text-slate-400 text-center mt-1">Format: JPG, PNG, dll</span>
            <span v-else class="text-[9px] text-emerald-500 font-bold text-center mt-1 truncate">{{ selectedFileName }}</span>
          </div>
        </div>
